...
%if isfield(TrialRecord.User,'reward schedule'), reward_schedule = TrialRecord.User.reward_schedule; else reward_schedule = [700 1800 1800 60 90]; end
if isfield(TrialRecord.User,'position'), position = TrialRecord.User.position; else position = [0 0]; end
if isfield(TrialRecord.User,'radius'), radius = TrialRecord.User.radius; else radius = 0.11; end
...
Color1 = [1 1 1];
Color2 = [0 0 0];
editable('reward_schedule_1', 'reward_schedule_2', 'fix_radius', 'SpatialFrequencyStep','TemporalFrequencyStep','-color',{'Color1','Color2'});

% scene1
fix1 = SingleTarget(eye_);
fix1.Target = 1;
fix1.Threshold = fix_radius;%2.5;%1.5;
...
rwd2a.Schedule = reward_schedule;%[200 500 500 60 90];%reward_schedule;%[100 800 800 60 90]; % you have to change this from trial record not here ********
...
% record keeping
trialerror(error_type);
TrialRecord.User.reward_schedule = rwd2a.Schedule;
