...
wrongChoice_delay = 1000; %1100;
fixBreak_delay = 500;
earlySaccade_delay = 1500; % Early saccade (Train monkey to think instead of randomly choosing)
...
% None: Randomly choose a condition
% AdaptiveBiasCorrection: correct for side and stimulus bias (side:
% leftvsright), stimulus(A vs B)
...
bhv_variable('fix_radius', fix_radius, ...
    ...
    'reward_schedule_type', reward_schedule_type, ...
...
if TrialRecord.User.initalCond
    ...
    if 0==error_type
        ...
        if strcmp(reward_schedule_type, 'Probabilistic')
        ...
        elseif strcmp(reward_schedule_type, 'ConsecutiveCorrect')
            errors = TrialRecord.TrialErrors;

            % include the current trial as correct, because error_type is 0 here
            recent_errors = [TrialRecord.TrialErrors 0];
            consecutive_correct = find(recent_errors ~= 0, 1, 'last');
            consecutive_correct = length(recent_errors) - max(consecutive_correct, 0);
        
            if consecutive_correct >= 4
                num_juice = 3;
            elseif consecutive_correct >= 3
                num_juice = 2;
            else
                num_juice = 1;
            end
