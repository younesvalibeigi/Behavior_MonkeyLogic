...
grab_800_natural_png()

function NAMES = grab_800_natural_png(outdir, save_images_mat)
...
zip_path   = fullfile(tempdir, 'DIV2K_train_HR.zip');
unz_dir    = fullfile(tempdir, 'DIV2K_train_HR_unz');

fprintf('Downloading DIV2K_train_HR.zip ...\n');
try
    websave(zip_path, div2k_url);
catch ME
    error('Failed to download DIV2K_train_HR.zip: %s', ME.message);
end
...
% 3) Find all PNGs, pick 800 at random (the set has exactly 100).
pngs = dir(fullfile(unz_dir, '**', '*.png'));
if numel(pngs) < 800
    error('Expected ~800 PNGs in DIV2K valid HR, found %d.', numel(pngs));
end
sel = randperm(numel(pngs), 800);

% 4) Copy to outdir with standardized names nat_001.png ... nat_100.png
codes = zeros(800, 4096);
...
fprintf('Done. Wrote 800 PNGs to %s\n', outdir);
end
