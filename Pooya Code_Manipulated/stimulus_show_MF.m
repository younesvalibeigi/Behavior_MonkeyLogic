%%
% These should be implemented: 1. gray background with right function
% 2. RF position, put different location in different blocks

%%

if ~exist('eye_','var'), error('This demo requires eye signal input. Please set it up or try the simulation mode.'); end
hotkey('x', 'escape_screen(); assignin(''caller'',''continue_'',false);');
bhv_code(21, 'Fixation', 30, 'Stimulus', 65, 'Reward')
...
stim_time = 200;
% Max_Trial = 3000;
% n_row = 1; % Number of Probe row
% grid_min = 0; % Grid LowerBound In DVA
% grid_max = 0;
% stim_size = 4; % STimulus size in DVA
% grid = linspace(grid_min, grid_max, n_row+1);
% gird_diff = grid(1,1:end-1);
% prob_location_matrix = permn(gird_diff,2);

prbe_stim_matrix = (2:11)'; % This is the number of TaskObjects
...
run_scene(scene1,30);     % Run the second scene (eventmarker 20)
...
run_scene(scene01);     % Run the second scene (eventmarker 20)
